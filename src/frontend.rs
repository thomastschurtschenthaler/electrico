...
impl Frontend {
    ...
    pub fn send_channel_message(&mut self, id:&String, channel:&String, args:&String) {
        if let Some(window) = self.windows.get(id) {
            let _ = window.webview.evaluate_script(format!("window.__electrico.sendChannelMessage('{}@{}');", channel, escape(args)).as_str());
        } else {
